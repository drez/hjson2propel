...
class Assets
{
	 ...
	 * @var string
	 */
	protected $public_dir = _PUBLIC_DIR;
	/**
	...
	/**
	 * Directory for local composer assets.
	 * Relative to your public directory ('public_dir').
	 * No trailing slash!.
	 *
	 * @var string
	...
	public function css($attributes = null)
	{
		...
		foreach ($assets as $asset)
			$output .= '<link href="' . _SITE_URL . $asset . '"' . $attributes . " />\n";
		return $output;
	}
	/**
	...
	public function js($attributes = null)
	{
		...
		$output = '';
		foreach ($assets as $asset)
			$output .= '<script src="' . _SITE_URL . $asset . '"' . $attributes . "></script>\n";
		return $output;
	}
	/**
	...
	protected function packLinks(array $links, Closure $minifier)
	{
		...
		foreach ($links as $link) {
			...
			$content = ($this->fetch_command instanceof Closure) ? $this->fetch_command->__invoke($link) : file_get_contents($link, false, stream_context_create($arrContextOptions));
			// Minify
	...
	{
		$composer = $this->assetIsFromComposer($asset);

		if ($composer !== false) {
			return $this->composer_dir . '/' . $composer[0] . '/' . $composer[1];
		}
	...
	protected function assetIsFromComposer($asset)
	{
		if (preg_match('{^(vendor)/(.*)$}', $asset, $matches))
			return array_slice($matches, 1, 3);
		return false;
	}
